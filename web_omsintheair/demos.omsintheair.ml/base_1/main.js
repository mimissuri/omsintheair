...
function new_route() {
    if (m_r) {
        document.getElementById("route_des").style.marginLeft = "calc(80% - 50px)";
        m_r = false;
    } else {
...
function cancel_route() {
    ...
    setTimeout(function () {
        mymap.setView([latitud, longitude], 13);
        document.getElementById("design_r").style.display = "block"
    }, 500);
...
$('#patata').terminal({
    ...
    help: function () {
        this.echo("Els commands disponibles són: ls, compile, reboot, echo i clear");
    },
}, {
    greetings: '[[b;#000;]OrangePi SSH]\nEscriu-re help per obtenir commands'
});